...
import torch
from models import CVaRResult, DrawdownAnalytics, MonteCarloResult, PortfolioWeights
...
def test_generate_correlated_gbm_paths(device):
    ...
    paths = generate_correlated_gbm_paths(
        initial, returns, vols, cov, days=10, num_paths=100, device=device
    )
...
def test_run_monte_carlo(sample_market_data):
    ...
    result, cvar, dd = run_monte_carlo(
        sample_market_data,
        ["511360", "511880"],
        weights,
        initial_prices,
