...
from monte_carlo import cholesky_decomposition, compute_max_drawdown
...
def _summarize_scenario(
    ...
    cvar_95 = (
        float(sorted_r[: max(cvar_idx_95, 1)].mean().item()) if cvar_idx_95 > 0 else 0.0
    )
...
def generate_fat_tails(
    ...
    return _summarize_scenario(
        portfolio_returns, portfolio_values, "fat_tails", n_paths
    )
...
def generate_regime_shift(
    ...
    drift_early = (annualized_rets * dt - 0.5 * annualized_vols**2 * dt).view(
        1, 1, n_assets
    )
    drift_late = (
        annualized_rets * crash_mean_mult * dt
        - 0.5 * (annualized_vols * crash_vol_mult) ** 2 * dt
    ).view(1, 1, n_assets)
    ...
    portfolio_returns, portfolio_values = _compute_portfolio_returns(all_paths, weights)
    return _summarize_scenario(
        portfolio_returns, portfolio_values, "regime_shift", n_paths
    )
...
def generate_corr_breakdown(
    ...
    corr_early = cov_matrix / torch.outer(annualized_vols, annualized_vols).clamp(
        min=1e-12
    )
    ...
    corr_breakdown = (
        corr_breakdown + torch.eye(n_assets, device=device) * 1e-4
    )  # jitter for PD

    L_early = cholesky_decomposition(corr_early)
    L_break = cholesky_decomposition(corr_breakdown)
    ...
    portfolio_returns, portfolio_values = _compute_portfolio_returns(all_paths, weights)
    return _summarize_scenario(
        portfolio_returns, portfolio_values, "corr_breakdown", n_paths
    )
