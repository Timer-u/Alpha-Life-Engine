...
def compute_sobol_indices(
    ...
    for i in range(d):
        ...
        for _ in range(n_boot):
            ...
            if var_y_boot <= 1e-15:
                continue
            si_boot = 2.0 * np.cov(YA_boot, y_ab_boot)[0, 1] / var_y_boot if len(YA_boot) > 1 else 0.0
            boot_si.append(np.clip(si_boot, -1.0, 1.0))

            sti_boot = 1.0 - 2.0 * np.cov(y_ab_boot, YB_boot)[0, 1] / var_y_boot if len(YB_boot) > 1 else 1.0
...
def _zero_result(param_names: list[str]) -> dict:
    return {
        ...
        "confidence_first": {p: (0.0, 0.0) for p in param_names},
        "confidence_total": {p: (0.0, 0.0) for p in param_names},
    }
