...
Usage:
    ...
    python scripts/local-evolver/evolver.py --api-url http://localhost:8787
...
from data import TRACKED_SYMBOLS, fetch_market_data
from config import load_config
from dsr import compute_haircut_sharpe
from models import EvolverConfig, StrategyParameterBounds
from report import generate_report, push_report_to_cloud, serialize_report


@click.command()
@click.option("--api-url", default="http://localhost:8787", show_default=True,
              help="Backend API base URL (without /api prefix)")
@click.option("--gbm-paths", default=10000, type=int, show_default=True,
              help="Number of Monte Carlo GBM paths")
@click.option("--wf-sets", default=200, type=int, show_default=True,
              help="Number of walk-forward parameter sets to evaluate")
@click.option("--frontier-pts", default=50, type=int, show_default=True,
...
def main(api_url: str, gbm_paths: int, wf_sets: int, frontier_pts: int, no_push: bool):
    ...
    click.echo(f"GPU available: {__import__('torch').cuda.is_available()}")
    if __import__('torch').cuda.is_available():
        click.echo(f"GPU device: {__import__('torch').cuda.get_device_name(0)}")

    click.echo("\nFetching market data from backend...")
    data = fetch_market_data(api_url)
    click.echo(f"  Loaded {len(data.symbols)} symbols")

    empty_symbols = [s for s in TRACKED_SYMBOLS if s not in data.symbols or not data.symbols[s].close]
    if empty_symbols:
        click.echo(f"  WARNING: {len(empty_symbols)} symbol(s) have no data: {empty_symbols}", err=True)
    if all(not data.symbols.get(s, DataFrame(dates=[], close=[], open=[], high=[], low=[], volume=[])).close for s in TRACKED_SYMBOLS):
        raise RuntimeError("All symbols have no market data. Cannot proceed with evolution.")
    ...
    click.echo(f"  Step 2/4: Monte Carlo stress test (GPU)...")
    ...
    click.echo(f"    Max drawdown: {report.monte_carlo_result.summary.max_drawdown:.4%}")
    ...
    click.echo(f"  Step 3/4: Walk-forward optimization (CPU)...")
    click.echo(f"    PBO score: {report.walk_forward_summary.pbo_score:.4f}")
    click.echo(f"    Stability score: {report.walk_forward_summary.stability_score:.4f}")
    click.echo(f"    Haircut Sharpe: {compute_haircut_sharpe(report.walk_forward_summary.dsr_rankings[0] if report.walk_forward_summary.dsr_rankings else 0.0, 200):.4f}")

    click.echo(f"  Step 4/4: Stability & regime check...")
    click.echo(f"    Gradient: {report.stability_report.gradient:.4f}")
    click.echo(f"    Stable: {report.stability_report.is_stable}")
    click.echo(f"    Regime: {report.regime_result.regime_label} (probs: {[f'{p:.2f}' for p in report.regime_result.regime_probs]})")
    click.echo(f"    Drift alert: {report.drift_result.alert} (PSI: {report.drift_result.psi:.4f})")

    click.echo(f"\nRecommended parameters:")
    p = report.recommended_params
    click.echo(f"  Trigger line: {p.trigger_line}")
    click.echo(f"  Safe ratio: {p.safe_ratio:.2f} / Ambition ratio: {p.ambition_ratio:.2f}")
