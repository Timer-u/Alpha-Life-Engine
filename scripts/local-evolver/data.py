...
def fetch_market_data(api_base_url: str) -> MarketDataInput:
    ...
    for row in rows:
        ...
        if sym not in by_symbol:
            by_symbol[sym] = {"dates": [], "open": [], "high": [], "low": [], "close": [], "volume": []}
        by_symbol[sym]["dates"].append(row["date"])
        by_symbol[sym]["open"].append(float(row["open"]) if row["open"] is not None else 0.0)
        by_symbol[sym]["high"].append(float(row["high"]) if row["high"] is not None else 0.0)
        by_symbol[sym]["low"].append(float(row["low"]) if row["low"] is not None else 0.0)
        by_symbol[sym]["close"].append(float(row["close"]) if row["close"] is not None else 0.0)
        by_symbol[sym]["volume"].append(int(row["volume"]) if row["volume"] is not None else 0)
