...
def cholesky_decomposition(matrix: torch.Tensor) -> torch.Tensor:
    try:
        ...
        try:
            ...
            print("[monte_carlo] Cholesky: matrix not positive definite, using eigenvalue regularization")
...
def covariance_to_correlation(cov_matrix: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
...
def generate_correlated_gbm_paths(
    ...
    drifts = (annualized_returns * dt - 0.5 * annualized_volatilities ** 2 * dt).view(1, 1, n_assets)
    ...
    for chunk_start in range(0, num_paths, chunk_size):
        ...
        z = torch.randn(chunk_paths, days, n_assets, device=device, dtype=initial_prices.dtype)
        correlated = z @ L.T
        ...
        chunk_prices[:, 1:, :] = initial_prices.view(1, 1, n_assets) * torch.exp(cum_returns)
...
def compute_drawdown_analytics(portfolio_values: torch.Tensor, n_paths: int) -> DrawdownAnalytics:
    ...
    annualized_ret = float((final_values / starting_values.clamp(min=1e-12)).mean().item())
...
def compute_monte_carlo_summary(
    ...
    summary = MonteCarloSummary(
        ...
        percentiles=MonteCarloPercentiles(
            ...
            p50=median_return, p75=q(0.75), p90=q(0.90), p95=q(0.95), p99=q(0.99),
        ),
    )

    cvar = CVaRResult(
        cvar_95=cvar_95, cvar_99=cvar_99, cvar_995=cvar_995,
        es_95=cvar_95, es_99=cvar_99, es_995=cvar_995,
    )
...
def run_monte_carlo(
    ...
    summary, cvar, dd_analytics = compute_monte_carlo_summary(portfolio_returns, portfolio_values)
