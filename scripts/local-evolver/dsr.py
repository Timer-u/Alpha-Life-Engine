...
def compute_annualized_sharpe(daily_returns: np.ndarray, risk_free_rate: float = 0.0) -> float:
...
def compute_dsr(
    ...
    denom_sq = 1.0 - skewness * sharpe + ((excess_kurtosis + 2.0) / 4.0) * sharpe * sharpe
    if denom_sq <= 0.0:
        return 0.0
...
def compute_sharpe_mean_std_ratio(sharpe_values: np.ndarray) -> float:
    return compute_sharpe_ratio(sharpe_values)


def compute_haircut_sharpe(sr: float, n_trials: int) -> float:
    """Bailey-Lopez de Prado haircut Sharpe ratio.
    对多重测试偏差进行惩罚：Sharpe 越高、试验次数越多，折扣越大。
    """
    if n_trials <= 1:
        return sr
    gamma = 0.5772156649  # Euler-Mascheroni constant
    heuristic_var = 1.0 / (n_trials ** 0.5)  # 简化版惩罚项
    E_max = (1 - gamma) * normal_inv_cdf(1 - 1.0 / n_trials) + gamma * normal_inv_cdf(1 - 1.0 / n_trials * math.e ** (-1))
...
def bootstrap_ci(
    ...
    maxdds = np.array([float(np.min((np.maximum.accumulate(b) - b) / np.maximum.accumulate(b))) for b in boot])
