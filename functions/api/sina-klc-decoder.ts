...
const SINA_KLC_DECODE = new Function(`${SINA_KLC_DECODER_SOURCE}\n;return d;`)() as (p: string) => unknown;

export function decodeSinaKlc(payload: string): SinaKlineRow[] {
  const rows = SINA_KLC_DECODE(payload);
  if (!Array.isArray(rows)) throw new Error('Sina response parse failed: decoder did not return an array');
  return rows as SinaKlineRow[];
}
